...
def _pivot_all(flat_rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for r in flat_rows:
        ...
        if not sku: continue
        g = out.setdefault(sku, {"name": r.get("product_name") or "N/A",
...
def _apply_only_promo(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [r for r in rows if _to_num(r.get("product_price_promo")) is not None]
...
def _apply_price_subset(rows: List[Dict[str, Any]], site_code: str, subset: PriceSubset) -> List[dict]:
    ...
    if site_code == "all":
        ...
        for sku, g in grouped.items():
            ...
            if our is None or min_comp is None: continue
    ...
    for r in rows:
        ...
        if our is None or comp is None: continue
        if subset == PriceSubset.ours_lower and our < comp: out.append(r)
        elif subset == PriceSubset.ours_higher and our > comp: out.append(r)
    return out
...
def _compose_stats(rows: List[Dict[str, Any]], site_code: str) -> Dict[str, int]:
    ...
    if site_code == "all":
        ...
        for _, g in grouped.items():
            ...
            if our is None or minc is None: no_comp += 1
            elif our < minc: ours_lower += 1
            elif our > minc: ours_higher += 1
            else: equal += 1
    else:
        ...
        for r in by_sku.values():
            ...
            our = _eff(r.get("product_price_promo"), r.get("product_price_regular"))
            comp = _eff(r.get("competitor_price_promo"), r.get("competitor_price_regular"))
            if our is None or comp is None: no_comp += 1
            elif our < comp: ours_lower += 1
            elif our > comp: ours_higher += 1
            else: equal += 1
    return {"total": total, "ours_lower": ours_lower, "ours_higher": ours_higher, "equal": equal, "no_comp": no_comp}

# =============================== Workbook utils ===============================
_HEADERS = ["ID","Product name","My price","Change","Min price comp.","Change","Diff","Competitor(s)"]
_WIDTHS  = [110, 720, 120, 110, 160, 110, 110, 280]
...
def _setup_sheet(ws):
    ...
    for i, h in enumerate(_HEADERS, start=1):
        ...
        c.alignment = _mid
        c.border = _border
        ws.column_dimensions[get_column_letter(i)].width = _px_to_w(_WIDTHS[i - 1])
...
def _fill_sheet(ws, rows: List[Dict[str, Any]], site_code: str):
    if site_code == "all":
        ...
        for sku, g in grouped.items():
            ...
            ws.append([
                sku, g["name"] or "N/A",
                round(our, 2) if our is not None else None, 0.0,
                round(min_comp, 2) if min_comp is not None else None, 0.0,
                round(diff, 2) if diff is not None else None,
                f"{sites[0]} (1 of {g['sites_count']})" if sites else "N/A",
            ])
            if urls:
        ...
        for r in rows:
            ...
            ws.append([
                sku, pname,
                round(our, 2) if our is not None else None, 0.0,
                round(comp, 2) if comp is not None else None, 0.0,
                round(diff, 2) if diff is not None else None,
                f"{r.get('competitor_site') or 'N/A'}" + (" (1 of 1)" if (r.get('competitor_site')) else "")
            ])
    ...
    for r in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=8):
        ...
        if my is not None and cmpv is not None:
            if my < cmpv: r[6].fill = _green
            elif my > cmpv: r[6].fill = _red
    ws.freeze_panes = "A2"
...
# NEW: union helper for multi-tag rules (dedupe by SKU + site identity)
...
def _build_and_send(rule: EmailRule) -> Dict[str, Any]:
    """
    One Excel file per rule.
    - Single tag (or no tag): 1 sheet with that slice.
    - Multiple tags: 1 sheet with the UNION of all slices (deduplicated).
    """
...
# Back-compat (UI used POST earlier)
...
async def _compute_next_run(now: datetime, schedule: dict) -> Optional[datetime]:
    ...
    for delta in range(0, 8):
        ...
        if not hhmm or hhmm in ("-","--",""): continue
